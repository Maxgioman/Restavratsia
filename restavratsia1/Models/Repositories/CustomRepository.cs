    ...
    public class CustomRepository
    {
        ...
        public List<Custom> GetCustoms(User user)
        {
            ...
            try
            {
                ...
                var isCompany = context.User.Where(x => x.Id == user.Id).Select(x => x.IsCompany);
                if (isCompany.Single() == 0)
                {
                    for(int count = listOfCustoms.Count - 1; count >= 0;count--)
                    {
                        if (listOfCustoms.ElementAt(count).UserId != user.Id)
                        {
                            listOfCustoms.RemoveAt(count);
